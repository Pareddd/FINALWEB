<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <a href="{{ route('events.create') }}" class="bg-fuchsia-600 hover:bg-fuchsia-700 text-white px-4 py-2 rounded text-sm font-bold transition shadow-lg w-full sm:w-auto text-center">
                + BUAT EVENT BARU
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg border border-white/10 p-4 sm:p-6">
                                ...
                                <tr class="hover:bg-slate-700/50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <a href="{{ route('events.show', $event) }}" class="text-cyan-400 hover:text-cyan-300 text-sm font-bold underline">Lihat</a>
                                            <a href="{{ route('events.edit', $event) }}" class="text-yellow-400 hover:text-yellow-300 text-sm font-bold underline">Edit</a>
                                            ...
                                            <form action="{{ route('events.destroy', $event) }}" method="POST" onsubmit="return confirm('Hapus event ini?');">
                                                @csrf @method('DELETE')
                                                <button class="text-red-500 hover:text-red-400 text-sm font-bold underline">Hapus</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
