...
<head>
    ...
    <title>SoundTix - Tiket Konser Musik</title>
...
<body class="antialiased bg-slate-900 text-white overflow-x-hidden">
    ...
    <div class="relative min-h-[70vh] flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div class="absolute inset-0 z-0">
            <img src="https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=2070&auto=format&fit=crop" alt="Concert Background" class="w-full h-full object-cover opacity-60">
            <div class="absolute inset-0 bg-gradient-to-b from-black/80 via-purple-900/40 to-slate-900"></div>
        </div>

        <div class="relative z-10 text-center max-w-4xl mx-auto w-full mt-10">
            ...
            <h1 class="font-display text-4xl sm:text-6xl md:text-8xl font-black text-white mb-6 leading-tight">
                FEEL THE <br/> <span class="neon-text">RHYTHM</span>
            </h1>
            
            <p class="text-gray-200 text-sm sm:text-lg mb-8 max-w-xl mx-auto font-light tracking-wide px-4">
            ...
            <form action="{{ route('home') }}" method="GET" class="w-full max-w-md sm:max-w-xl mx-auto relative group px-2">
                ...
                <div class="relative flex flex-col sm:flex-row bg-black rounded-lg border border-white/20 p-1">
                    <input type="text" name="search" placeholder="Cari Artis, Genre, atau Kota..." 
                           class="w-full bg-transparent text-white border-none focus:ring-0 px-4 py-3 placeholder-gray-500 outline-none text-center sm:text-left mb-2 sm:mb-0 text-sm"
                           value="{{ request('search') }}">
                    <button type="submit" class="bg-white text-black font-black px-6 py-2 rounded hover:bg-gray-200 transition uppercase tracking-widest text-xs sm:text-sm w-full sm:w-auto">
                        CARI
                    </button>
                </div>
            </form>
    ...
    <div class="py-16 sm:py-24 relative bg-slate-900 px-4 sm:px-6 lg:px-8">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="flex justify-between items-end mb-12 border-b border-white/10 pb-4">
                <div>
                    <h2 class="text-2xl sm:text-4xl font-display font-bold text-white mb-2">UPCOMING <span class="text-fuchsia-500">EVENTS</span></h2>
                </div>
            ...
            @if($events->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    ...
                    <div class="group relative bg-gray-800 rounded-xl overflow-hidden hover:-translate-y-2 transition-all duration-300 shadow-2xl border border-white/5 flex flex-col h-full">
                        ...
                        <div class="p-4 sm:p-6 relative z-20 flex flex-col flex-grow -mt-2">
                            <span class="text-[10px] font-bold tracking-widest text-blue-400 uppercase mb-1 block">{{ $event->kategori }}</span>
                            
                            <h3 class="text-lg sm:text-xl font-bold text-white mb-2 font-display leading-tight group-hover:text-fuchsia-400 transition line-clamp-2">
                                {{ $event->name }}
                            </h3>
                            
                            <div class="flex items-center text-gray-400 text-xs sm:text-sm mb-4">
                            ...
                            <div class="mt-auto flex items-center justify-between border-t border-white/10 pt-3">
                                <div>
                                    <span class="text-[10px] text-gray-500 block uppercase">Mulai Dari</span>
                                    ...
                                    @else
                                        <span class="text-sm font-bold text-white">TBA</span>
                                    @endif
                                </div>
                                
                                <a href="{{ route('events.show', $event) }}" class="bg-white text-black hover:bg-fuchsia-500 hover:text-white px-4 py-2 rounded-full font-bold text-xs transition-colors duration-300 shadow-lg">
                                    TIKET
                                </a>
                            </div>
                        </div>
    ...
    <footer class="bg-black border-t border-white/10 py-12 pb-8 mt-12">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <h2 class="font-display text-2xl font-bold text-white mb-4">SOUND<span class="text-fuchsia-500">TIX</span></h2>
            <p class="text-white-600 text-xs">&copy; {{ date('Y') }} SoundTix Indonesia. Keep The Music Alive! 🤘</p>
        </div>
    </footer>
